...
if [[ $# -eq 2 ]]; then
    ...
    if [[ ! -d "$coreth_path" ]]; then
        go get "github.com/Toinounet21/coreth-trafficked-v0.8.5-rc.0@$coreth_version"
    fi
else
...
cd "$coreth_path"
go build -ldflags "-X github.com/Toinounet21/coreth-trafficked-v0.8.5-rc.0/plugin/evm.Version=$coreth_version $static_ld_flags" -o "$evm_path" "plugin/"*.go
cd "$AVALANCHE_PATH"
